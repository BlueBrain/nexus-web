...
const Tools = ({ tools, onAddTool }: ToolsProps) => {
  const [toolDescriptionExpanded, setToolDescriptionExpanded] = React.useState<boolean[]>();

  React.useEffect(() => tools && setToolDescriptionExpanded(tools.map((i, ix) => false)), [tools]);
  ...
  if (tools === undefined || tools.length === 0) {
    return (
      <section>
        <h3>Tools</h3>
        <p>
          Hello there, We are missing the script used to generate this report. Fancy helping us
          making more sense of the following information?
        </p>
          ...
          >
            <div>
              <div style={{ display: 'flex' }}>
                <div>
                  <h4 style={{ textTransform: 'uppercase', color: '#BFBFBF' }}>Code Repository</h4>
                  <p>
              ...
              {t.description && toolDescriptionExpanded && toolDescriptionExpanded[ix] && (
                <>
                  <h4 style={{ textTransform: 'uppercase', color: '#BFBFBF' }}>
                    How did we run our tool
                  </h4>
                  <p style={{ color: '#262626', whiteSpace: 'pre-wrap' }}>{t.description}</p>
                </>
              )}
            </div>
          </li>
