...
export const makeNode = async (
  ...
  let resourceData;
  if (!isExternal) {
    const { orgLabel, projectLabel } = getOrgAndProjectFromResource(link as Resource)!;
...
export const createNodesAndEdgesFromResourceLinks = (
  ...
  return resourceLinks.reduce((pathNodes: cytoscape.ElementDefinition[], link) => {
    ...
    const edges = paths.map((path, index) => {
      ...
      return {
        data: {
          label,
          id: `edge-${prev && prev.data.id}-${blankNode && blankNode.data.id}`,
          source: prev && prev.data.id,
          target: blankNode && blankNode.data.id,
          parentId: originId,
        },
...
export const makeBlankNodes = (path: string, resourceId: string, linkId: string) => {
  return {
    data: {
...
export const getListOfChildrenRecursive = (
  ...
  const childIds = elements
    .filter((element) => element.data.source === parentId || element.data.parentId === parentId)
    .map((child) => child.data.id)
    .filter(Boolean) as string[];

  const childRecursiveIds = childIds.reduce((childRecursiveIdsList: string[], id: string) => {
    return [...childRecursiveIdsList, ...getListOfChildrenRecursive(id, elements)];
  }, []);
