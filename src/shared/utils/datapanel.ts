...
const baseLocalStorageObject = (
  ...
  return {
    ...
    type: isArray(resource['@type']) ? resource['@type'] : [resource['@type'] ?? ''],
  };
};

export const toLocalStorageResources = (resource: Resource, source: string): TDataSource[] => {
      ...
      return [
        {
          ...
          distribution: {
            ...
            encodingFormat: resource['@type'] === 'File' ? resource._mediaType : 'json',
            label: resource['@type'] === 'File' ? resource._filename : 'metadata.json',
          },
        },
      ];
    }
      ...
      resource.distribution.forEach((distItem, index) => {
        localStorageObjs.push({
          ...
          distribution: {
            ...
            contentSize:
              (distItem.contentSize as { value: number })?.value ?? distItem.contentSize ?? 0,
      ...
      // Now store an object for the distribution item.
      {
        ...
        distribution: {
          ...
          contentSize: isArray(resource.distribution?.contentSize)
            ...
            : resource.distribution?.contentSize?.value ?? resource.distribution?.contentSize ?? 0,
          ...
          label: fileNameForDistributionItem(resource.distribution, resourceName),
        },
      },
    ];
  } catch (err) {
...
export const distributionName = (distItem: any, defaultName: string) => {
  const distName: string =
    distItem?.label ?? distItem?.name ?? distItem?._filename ?? distItem?.filename ?? defaultName;
  return distName;
};

export const fileNameForDistributionItem = (distItem: any, defaultName: string) => {
  ...
  const distNameHasExtension = Boolean(
    !isValidUrl(distName) &&
      distName.slice(distName.includes('.') ? distName.lastIndexOf('.') : distName.length)
  );

  if (!distItem?.name || !distNameHasExtension) {
    Sentry.captureMessage('Distribution item does not have name or extension.', {
  ...
  const fileName = distNameHasExtension
    ? distName
    : `${distName}.${fileExtensionFromResourceEncoding(distItem?.encodingFormat)}`;

  return fileName;
};

export const removeLocalStorageRows = (rows: TDataSource[], keysToRemove: string[]) => {
  ...
  return rows.filter((row) => !keysToRemove.find((key) => row.key.toString().startsWith(key)));
};

export const distributionMatchesTypes = (distItem: any, fileExtenstions: string[]): boolean => {
  ...
  return sum(
    compact(
      flatMap(resultsObject).map((item) => {
        ...
        return distributionMatchesTypes(item?.distribution, types) ? item?.size : 0;
      })
    )
  );
};
...
export function pathForTopLevelResources(
  ...
  return {
    ...
    extension: resource['@type'] === 'File' && resource.contentType ? resource.contentType : 'json', // Max Length - 4
  };
}
