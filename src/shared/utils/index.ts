...
/**
 * True if given date is today, false otherwise
 */
...
/**
 * How long ago from today in days
 *
 * return -1 if given date if from the future
 *
 */
export function howLongAgo(date: string): number {
  const today = new Date(Date.now());
  const givenDate = new Date(date);
