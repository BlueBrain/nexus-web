        ...
        try {
          ...
          const size = resource.distribution
            ? isArray((resource.distribution as TDistribution)?.contentSize)
              ? sum(
                  ...((resource.distribution as TDistribution)
          ...
          const contentType = resource.distribution
            ? isArray((resource.distribution as TDistribution).label)
              ? (resource.distribution as TDistribution)?.label[0]
                  .split('.')
                  .pop()
              : ((resource.distribution as TDistribution)?.label as string)
                  ?.split('.')
                  .pop() ?? ''
            : '';
          const type =
  ...
  const totalSize = sum(
    ...compact(
      flatMap(resultsObject).map(item =>
        item?.size
          ? isArray(item?.size as number[])
            ? sum(...(item?.size as number[]))
            : (item?.size as number)
          : 0
      )
    )
