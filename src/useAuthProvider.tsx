...
export const setupUserSession = async (userManager: UserManager) => {
    ...
    userManager.events.addUserLoaded(async user => {
        ...
        const identities = await ((await fetch(`${store.getState().config.apiEndpoint}/identities`))).json();
        store.dispatch(fetchIdentitiesFulfilledAction(identities));
    });

    // Raised prior to the access token expiring.
    userManager.events.addAccessTokenExpiring(() => {
        console.debug('@@addAccessTokenExpiring')
    ...
    userManager.events.addAccessTokenExpired(() => {
        console.debug('@@addAccessTokenExpired')
    ...
    userManager.events.addSilentRenewError((error: Error) => {
        console.debug('@@addSilentRenewError')
    ...
    userManager.events.addUserSignedOut(() => {
        console.debug('@@addUserSignedOut')
