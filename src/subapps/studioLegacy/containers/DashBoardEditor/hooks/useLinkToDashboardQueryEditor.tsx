...
const useLinkToDashboardQueryEditor = (viewId: string, orgLabel: string, projectLabel: string) => {
  ...
  React.useEffect(() => {
    nexus.View.get(orgLabel, projectLabel, encodeURIComponent(viewId)).then((result) => {
      setView(result);
    });
  }, [viewId]);

  const linkQueryEditor = React.useMemo<(dataQuery: string) => React.ReactElement>(() => {
