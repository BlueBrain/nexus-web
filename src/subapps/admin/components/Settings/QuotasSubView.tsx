...
const GaugeComponent = ({ percent, total, quota, name }: GaugeProps) => {
    return (<div className='quotas-gauge-chart'>
        <Gauge
        ...
        />
        <div className='qgauge-description'>Total: {total} {name} </div>
        <div className='qgauge-description'>Quota: {quota} {name} </div>
    </div>
    )
}

const QuotasSubView = (props: Props) => {
    const resources: GaugeProps = { name: 'Resources', quota: 100000, total: 89000, percent: .89 }
    const events: GaugeProps = { name: 'Events', quota: 30000, total: 11000, percent: .34 }
